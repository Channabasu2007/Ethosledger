...
export async function GET(request) {
  ...
  try {
    ...
    // ========================================
    // STEP 1: LIGHTHOUSE VIA PAGESPEED INSIGHTS API
    // ========================================
    let lighthouseData = null;
    try {
      console.log('🔦 Fetching Lighthouse data for:', targetUrl);

      const apikey = "AIzaSyBEaclWYS-8miAiSxcM3T8CFkSIKWfWjr4";
      const pagespeedUrl = `https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(targetUrl)}&category=performance&category=accessibility&category=best-practices&category=seo&strategy=desktop&key=${apikey}`;

      const psResponse = await fetch(pagespeedUrl, {
        headers: { 'User-Agent': 'SustainCode-Audit-Tool/1.0' }
      });

      if (!psResponse.ok) {
        throw new Error(`PageSpeed API returned ${psResponse.status}`);
      }
      ...
      if (psData.lighthouseResult) {
        ...
        lighthouseData = {
          ...
          metrics: {
            fcp: `${((lhr.audits?.['first-contentful-paint']?.numericValue || 0) / 1000).toFixed(2)}s`,
            lcp: `${((lhr.audits?.['largest-contentful-paint']?.numericValue || 0) / 1000).toFixed(2)}s`,
            tbt: `${Math.round(lhr.audits?.['total-blocking-time']?.numericValue || 0)}ms`,
            cls: (lhr.audits?.['cumulative-layout-shift']?.numericValue || 0).toFixed(3),
            si: `${((lhr.audits?.['speed-index']?.numericValue || 0) / 1000).toFixed(2)}s`,
            tti: `${((lhr.audits?.['interactive']?.numericValue || 0) / 1000).toFixed(2)}s`,
          },
          opportunities: Object.values(lhr.audits || {})
            ...
            .map(audit => ({
              ...
              savings: audit.displayValue || 'Unknown',
            })),
        };
        console.log('✅ Lighthouse data processed');
      }
    } catch (lighthouseError) {
      console.error('❌ PageSpeed Insights failed:', lighthouseError.message);
      lighthouseData = {
        scores: { performance: 0, accessibility: 0, bestPractices: 0, seo: 0 },
        metrics: { fcp: '0s', lcp: '0s', tbt: '0ms', cls: '0', si: '0s', tti: '0s' },
        opportunities: [],
    ...
    else if (seoScore >= 80) seoGrade = 'B';
    else if (seoScore >= 70) seoGrade = 'C';
    else if (seoScore >= 60) seoGrade = 'D';

    // ========================================
    // STEP 4: SECURITY/DANGEROUS CODE ANALYSIS
    // ========================================
    const dangerousCodeData = await page.evaluate(() => {
      ...
      const unsafeIframes = Array.from(iframes).filter(iframe => {
        return !iframe.hasAttribute('sandbox') || iframe.src.startsWith('javascript:');
      });
    ...
    let securityGrade = 'F';
    if (dangerScore >= 90) securityGrade = 'A';
    else if (dangerScore >= 80) securityGrade = 'B';
    else if (dangerScore >= 70) securityGrade = 'C';
    else if (dangerScore >= 60) securityGrade = 'D';

    // ========================================
    // STEP 5: CPU & CARBON METRICS
    // ========================================
    ...
    const CARBON_INTENSITY = 442;
    const GREEN_HOSTING_FACTOR = 0.5;

    const mb = totalBytes / (1024 * 1024);
    const gb = mb / 1024;

    const dataTransferEnergy = gb * ENERGY_PER_GB * GREEN_HOSTING_FACTOR;
    const cpuEnergy = (parseFloat(cpuPercent) / 100) * 0.001;
    const totalEnergyKwh = dataTransferEnergy + cpuEnergy;
    const co2Grams = totalEnergyKwh * CARBON_INTENSITY;

    // CORRECTED SCORING
    let sustainabilityScore;
    if (co2Grams <= 0.1) {
      sustainabilityScore = 100;
    } else if (co2Grams <= 0.5) {
      sustainabilityScore = Math.round(95 - ((co2Grams - 0.1) / 0.4) * 15);
    } else if (co2Grams <= 1.0) {
      sustainabilityScore = Math.round(80 - ((co2Grams - 0.5) / 0.5) * 20);
    } else if (co2Grams <= 2.0) {
      sustainabilityScore = Math.round(60 - ((co2Grams - 1.0) / 1.0) * 30);
    } else if (co2Grams <= 5.0) {
      sustainabilityScore = Math.round(30 - ((co2Grams - 2.0) / 3.0) * 20);
    } else {
      sustainabilityScore = Math.max(0, Math.round(10 - ((co2Grams - 5.0) / 5.0) * 10));
    }

    let sustainabilityGrade = 'F';
    if (sustainabilityScore >= 90) sustainabilityGrade = 'A';
    else if (sustainabilityScore >= 75) sustainabilityGrade = 'B';
    else if (sustainabilityScore >= 50) sustainabilityGrade = 'C';
    else if (sustainabilityScore >= 25) sustainabilityGrade = 'D';

    const sustainability = {
      source: 'websitecarbon_methodology',
      model: 'swdv3_adjusted',
      totalBytes: totalBytes,
      megabytes: Number(mb.toFixed(2)),
      gigabytes: Number(gb.toFixed(6)),
      energyKwh: Number(totalEnergyKwh.toFixed(6)),
      co2Grams: Number(co2Grams.toFixed(3)),
      score: sustainabilityScore,
      grade: sustainabilityGrade,
      trackerCount: detectedTrackers.size,
      apiCalls: apiCallsCount,
      cpuContribution: Number((cpuEnergy * CARBON_INTENSITY).toFixed(3)),
      dataContribution: Number((dataTransferEnergy * CARBON_INTENSITY).toFixed(3))
    };
    ...
    try {
      ...
      await Site.findOneAndUpdate({ url: targetUrl }, sitePayload, { upsert: true });
    } catch (dbErr) {
    ...
    return NextResponse.json({
      ...
      timeline: [
        { year: "2020", size: (mb * 0.45).toFixed(2), co2: (co2Grams * 0.45).toFixed(3) },
        { year: "2023", size: (mb * 0.75).toFixed(2), co2: (co2Grams * 0.75).toFixed(3) },
        { year: "2026", size: mb.toFixed(2), co2: co2Grams.toFixed(3) },
      ],

      // Benchmarks for user reference
      benchmarks: {
        excellent: { threshold: '< 0.1g', score: '95-100', grade: 'A' },
        good: { threshold: '0.1-0.5g', score: '80-95', grade: 'B' },
        average: { threshold: '0.5-1.0g', score: '60-80', grade: 'C' },
        poor: { threshold: '1.0-2.0g', score: '30-60', grade: 'D' },
        critical: { threshold: '> 2.0g', score: '0-30', grade: 'F' }
      }
    });
