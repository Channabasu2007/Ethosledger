...
export default function CodeAnalysisWidget({ trackers = [] }) {
    return (
        ...
        <div className="col-span-1 md:col-span-2 bg-[#0d131f]/80 backdrop-blur-2xl rounded-[32px] outline outline-1 outline-white/10 overflow-hidden relative min-h-[315px] flex flex-col group transition-all duration-500 hover:outline-red-500/30 hover:shadow-[0_0_50px_-12px_rgba(239,68,68,0.3)]">
            ...
            <div className="p-6 font-mono text-[12px] leading-relaxed overflow-hidden flex-1 relative z-20 custom-scrollbar lowercase">
                <div className="flex gap-4 opacity-20 group-hover:opacity-40 transition-opacity duration-700">
                    <span className="w-6 text-right text-gray-500 select-none">01</span>
                    <span className="text-blue-400 text-xs">detecting <span className="text-white">trackers</span>...</span>
                </div>

                {trackers.length > 0 ? (
                    trackers.map((t, i) => (
                        <div key={i} className="flex gap-4 my-1 py-1 group/line overflow-hidden">
                            <span className="w-6 text-right text-gray-600 font-bold select-none z-10 text-[11px]">{String(i + 2).padStart(2, '0')}</span>
                            <span className="text-red-400 font-bold z-10 text-[11px] truncate flex gap-2 items-center">
                                <span className="text-white bg-red-500/20 px-1 text-[9px] uppercase">{t.category}</span>
                                {t.name}
                            </span>
                        </div>
                    ))
                ) : (
                    <div className="flex gap-4 my-2 py-2 px-3 relative group/line overflow-hidden rounded-r-lg">
                        <span className="w-6 text-right text-gray-600 font-black select-none z-10 text-[11px]">02</span>
                        <span className="text-green-400 font-bold z-10 text-[11px] truncate">
                            No active trackers found.
                        </span>
                    </div>
                )}

                {/* 5. Floating Alert Card */}
                {trackers.length > 0 && (
                    <div className="mt-4 p-4 rounded-[20px] bg-gradient-to-br from-[#161b22] to-[#0d1117] border border-red-500/20 shadow-xl flex items-center gap-4 transform group-hover:-translate-y-1 transition-transform">
                        ...
                        <div className="min-w-0">
                            <p className="text-white font-black italic text-[11px] tracking-tight uppercase truncate">Surveillance Detected</p>
                            <p className="text-[10px] text-gray-400 mt-0.5 leading-tight font-medium line-clamp-2">
                                {trackers.length} tracking modules found.
                            </p>
                        </div>
                    </div>
                )}
            ...
            <div className="px-6 py-3 border-t border-white/5 bg-black/40 backdrop-blur-md flex items-center justify-between relative z-20">
                <div className="flex items-center gap-4">
                    <div className="flex flex-col">
                        ...
                        <span className={`text-[9px] font-bold uppercase mt-1 ${trackers.length > 0 ? 'text-red-500' : 'text-green-500'}`}>
                            {trackers.length > 0 ? 'Compromised' : 'Secure'}
                        </span>
                    </div>
                </div>
                {trackers.length > 0 && <div className="w-2 h-2 rounded-full bg-red-500 animate-ping opacity-30"></div>}
            </div>
